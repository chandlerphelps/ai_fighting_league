...
import type { DaySimulationResult, MatchResult } from '../types/world_state'
...
export default function Home() {
  ...
  const beltHolder = ws.belt_holder_id
  const beltHolderName = ws.belt_history?.find(
    b => b.fighter_id === beltHolder && !b.lost_date
  )
  const currentDefenses = beltHolderName?.defenses ?? 0

  const recentMatches = ws.recent_matches?.slice(-20).reverse() || []
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: spacing.md }}>
        <InfoCard label="Belt Holder" value={beltHolder ? '' : 'VACANT'}>
          {beltHolder && (
            <div>
              <div style={{ color: colors.accentBright, fontSize: fontSizes.md }}>
                {ws.recent_matches?.find(m => m.fighter1_id === beltHolder || m.fighter2_id === beltHolder)
                  ? findFighterName(ws.recent_matches, beltHolder) : beltHolder}
              </div>
              <div style={{ color: colors.textMuted, fontSize: fontSizes.xs }}>
                {currentDefenses} defense{currentDefenses !== 1 ? 's' : ''}
              </div>
            </div>
          )}
        </InfoCard>
      ...
      {ws.promotion_fights && ws.promotion_fights.length > 0 && (
        <div style={{
          padding: spacing.md,
          ...
          <div style={{ color: colors.rivalry, fontSize: fontSizes.md, fontWeight: 'bold', marginBottom: spacing.sm }}>
            Promotion/Relegation Fights Scheduled ({ws.promotion_fights.length} matchups)
          </div>
          {ws.title_fight && ws.title_fight.champion_id && (
            <div style={{ color: colors.accent, fontSize: fontSizes.sm }}>
      ...
      {lastResult && lastResult.matches.length > 0 && (
        <div>
          ...
          <MatchList matches={lastResult.matches} />
          ...
          <div style={{
            ...
            {ws.season_logs.slice(-10).reverse().map(log => (
              <div key={log.season} style={{
                ...
                <div style={{ color: colors.accent, fontSize: fontSizes.sm }}>{log.belt_holder_name}</div>
...
function findFighterName(matches: MatchResult[], fighterId: string): string {
  for (const m of matches) {
    if (m.fighter1_id === fighterId) return m.fighter1_name
    if (m.fighter2_id === fighterId) return m.fighter2_name
  }
  return fighterId
}
...
function MatchList({ matches }: { matches: MatchResult[] }) {
  ...
  const tierOrder = ['championship', 'contender', 'underground']
